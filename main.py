...
def run_home() -> None:
    ...
    current_user = 'admin'  # admin or client
    chk = True
    screen_type = 0  # screen type can be 0/1/2 if main screen or instruction set.

    queue_lst = get_cities()    # Contains all cities
    current_index = 0   # index for queue_lst
    current_opt = 'use'  # 'use' a pre-existing map or 'create' a new map
    ...
    while chk:
        ...
        _display_correct_screen(screen_type=screen_type, screen=screen, queue_lst=queue_lst,
                                current_user=current_user, current_opt=current_opt,
                                current_index=current_index, enter_city_rect=enter_city_rect)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                current_user, current_opt = _handle_up_down_key(event=event,
    ...
    if current_opt == 'use' and queue_lst:
...
def _display_correct_screen(screen_type: int, screen: pygame.surface, queue_lst: list[str],
                            current_user: str, current_index: int, current_opt: str,
                            enter_city_rect: pygame.Rect) -> None:
    """Helper function to ...
    """
    if screen_type == 0:
...
def _handle_up_down_key(event: pygame.event, screen_type: int,
                        current_user: str, queue_lst: list[str],
                        current_opt: str) -> tuple[str, str]:
    """Helper function to ..."""
    ...
    return (current_user, current_opt)
...
def refresh_display(screen: pygame.Surface, screen_type: int, queue_lst: list = None,
                    current_user: str = None, current_index: int = None, current_opt: str = None,
