...
\begin{document}
    ...
    \\
    The map administrator will be able to develop the map of that region such as Dubai or Toronto and have the option to edit it. This will be implemented using the \texttt{pygame} package where the screen will be converted into small grids (similar to the linked list assignment but a lot smaller).
%The size of these grids will be according to a scale based on the size of the transit map and the administrator will be responsible for ensuring this is accurate.
    The administrator will have the ability to edit the map by having options like adding a new station and adding an edge to connect two stations (with corners to portray a realistic map), all drawn to scale.
%The map administrator will also be responsible for color-coding the lines and also updating the system signifying which paths are not feasible due to factors such as maintenance, traffic, etc.
